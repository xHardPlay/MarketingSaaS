...
function displayRowsSummary(rows) {
    rowsSummary.innerHTML = '<table><tr><th>Row</th><th>Client Company Name</th><th>Job Description</th><th>Slogan</th><th>ImageFileName</th><th>LogoFile</th><th>Action</th></tr></table>';
...
function selectRow(index) {
    ...
    if (row) {
        // Set slogan as canvas text and UI input
        const slogan = row.Slogan || '';
        textInput.value = slogan;

        // Update the initial text element if it exists
        if (selectedElement && selectedElement.type === 'text') {
            selectedElement.text = slogan;
        ...
        if (logoFile) {
            ...
            logoImg.onload = () => {
                ...
                // Re-select the initial text element
                const textElement = elements.find(el => el.type === 'text');
                if (textElement) {
                    textElement.isSelected = false; // Don't select both
                }
            ...
            // If no logo file specified, remove any existing logo elements
            elements = elements.filter(el => el.type === 'text');
            selectedElement = elements.find(el => el.type === 'text') || null;
            renderCanvas();
        }
